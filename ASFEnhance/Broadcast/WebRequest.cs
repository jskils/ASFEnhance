...
internal static class WebRequest
{
    ...
    /// <returns></returns>
    internal static async Task<GenericResponse<int?>> Watch(this Bot bot, ulong steamId, ulong seconds = 900)
    {
        // 获取直播间信息
    ...
    /// <param name="steamId"></param>
    /// <returns></returns>
    internal static async Task<JsonObject?> GetBroadcastMpd(this Bot bot, ulong steamId)
    {
        ...
        var response = await bot.ArchiWebHandler
            .UrlPostToJsonObjectWithSession<JsonObject>(request, data: data, referer: referer, headers: RequestHeader.UserAgentHeader).ConfigureAwait(false);
    ...
    /// <returns></returns>
    internal static async Task<bool> Heartbeat(this Bot bot, ulong steamId, string broadcastid, string viewertoken)
    {
        Uri referer = new(ArchiWebHandler.SteamCommunityURL, $"/broadcast/watch/{steamId}");
        ...
        var response = await bot.ArchiWebHandler.UrlPostToJsonObjectWithSession<JsonObject>(request, data: data, referer: referer, headers: RequestHeader.UserAgentHeader).ConfigureAwait(false);
    ...
    /// <returns></returns>
    internal static async Task<GenericResponse<int?>> Chat(this Bot bot, ulong steamId, string message)
    {
        // 获取直播间信息
        var broadcastInfo = await GetBroadcastMpd(bot, steamId).ConfigureAwait(false);
        if (broadcastInfo == null)
    ...
    /// <param name="broadcastid"></param>
    /// <returns></returns>
    internal static async Task<JsonObject?> GetChatInfo(this Bot bot, ulong steamId, string broadcastid)
    {
        Uri request = new(ArchiWebHandler.SteamCommunityURL, $"/broadcast/getchatinfo?steamid={steamId}&broadcastid={broadcastid}");
        Uri referer = new(ArchiWebHandler.SteamCommunityURL, $"/broadcast/watch/{steamId}");
        var response = await bot.ArchiWebHandler.UrlGetToJsonObjectWithSession<JsonObject>(request, referer: referer, headers: RequestHeader.UserAgentHeader).ConfigureAwait(false);
        var result = response?.Content;
        if (result == null || result["success"]!.GetValue<int>() != 1)
        {
    ...
    /// <param name="message"></param>
    /// <returns></returns>
    internal static async Task<bool> SendChatMsg(this Bot bot, string token, string chatId, string message)
    {
        ...
        var response = await bot.ArchiWebHandler
            .UrlPostToJsonObjectWithSession<JsonObject>(request, referer: ArchiWebHandler.SteamCommunityURL, data: data, headers: RequestHeader.UserAgentHeader, session: ArchiWebHandler.ESession.None)
